...
export default (parentNode: any, element: any, jnpfFlowInfo: any, injector: any) => {
  ...
  if (typeConfig && typeConfig[type]) {
    ...
    let name: any;
    if(jnpfFlowInfo.isFreeType && data?.type === typeTask && typeof data?.oneSelfEndApproval === "boolean") {
      name = data.oneSelfEndApproval ? '审批人员自行结束' : `审批节点流转${data.approvalNumber}次结束`
    }
    let nodeContent = element.elementBodyName || nodeMap.get(element.id)?.userName || name || data?.content || bodyDefaultText;
    if (isPreview) {
